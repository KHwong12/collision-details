...
devtools::install_github("Hong-Kong-Districts-Info/hkdatasets@f6df2ef48e62d5da1d643e04e266dc876d488f37")
```
...
hk_accidents_selected = filter(hk_accidents, Serial_No_ == params$SELECTED_ACCIDENT_NO)

```


## Detail Report of Accident No. `r as.character(params$SELECTED_ACCIDENT_NO)`
...
## Details of the accident {.tabset }
...
Full details of the accident is shown below.
...
```{r}
hk_vehicles_selected = filter(hk_vehicles, Serial_No_ == params$SELECTED_ACCIDENT_NO)
```

```{r}
hk_vehicles_selected %>%
  select(-c(OBJECTID, Serial_No_, Year)) %>%
...
```{r}
hk_casualties_selected = filter(hk_casualties, Serial_No_ == params$SELECTED_ACCIDENT_NO)
```

```{r}
hk_casualties_selected %>% 
  select(-c(OBJECTID, Serial_No_, Year, starts_with("Location_of_Injury_"))) %>%
  # do not add thousand separators as year are now in double type
